...
function renderContactSheet() {
  ...
  for (let i = 0; i < TOTAL_DAYS; i++) {
    ...
    const isCompleted = state && state.gameOver;
    ...
    const dateLabel = i === 0 ? "Today" : i === 1 ? "Yesterday" : dateObj.toLocaleDateString(undefined, { month: "short", day: "numeric" });

    const link = `index.html?day=${day}&cat=${activeTab}`;
    const frame = document.createElement("a");
    frame.href = link;
    frame.className = "film-strip-unit";
    ...
    if (isCompleted) {
      ...
      frame.innerHTML = `
        ...
        <div class="frame-center">
          <div class="sealed-negative-thumb">🎞️</div>
          <div class="frame-info">
            <div class="edge-code">
              <span>SAFETY FILM</span>
              <span>EXP·${day}</span>
            </div>
            <div class="frame-title" style="color: var(--text-muted, #94a3b8);">Classified Reel</div>
            <div class="frame-sub">${dateLabel} · Case #${day}</div>
            <span class="wax-stamp sealed">SEALED NEGATIVE</span>
          </div>
        </div>
        <div class="sprocket-rail">
          <div class="sprocket-hole"></div>
